	...
	export default {
		...
		methods: {
			initVideo () {
				...
				if ( !this._getScriptDomByDataId('hls') ) {
					this.dom.setAttribute('class', 'yb-video yb-video-first')
					this._initHLS().then(() => {
						this._initFLV().then(() => {
							this._initYBBarrage().then(() => {
								this._initYBPlayer().then(() => {
									this.switchVideo()
			...
			switchVideo () {
				try{
					if ( this.videoProp.src || this.videoProp.segments.length > 0 ) {
						if ( YBPlayer && YBBarrage && flvjs && Hls ) {
							if ( !this.dp && this.dom ) {
